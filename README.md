...
### Soubory a adresáře:
- **`axelar_downloader.py`**: Skript slibující stažení GMP (General Message Passing) transakcí z API Axelarscanu s ohledem na stránkovací limity. Obsahuje módy pro stahování konkrétní transakce, vyhledávání záznamů a dávkové stahování (batchování) a exportuje je primárně do CSV/JSON.
...
- **`ibc.ipynb`**: Pokročilý analytický zápisník, který zpracovává výstupy z IBC a připravuje datové agregace a grafy (CDF rozdělení atp.) pro text bakalářské práce.
